...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" exclude-result-prefixes="xs xd fsul" version="2.0">
    ...
    <xd:doc scope="stylesheet">
        <xd:desc>
            ...
            <xd:p><xd:b>Title: </xd:b>Defense Statements Remediation</xd:p>
    ...
    <xsl:template name="defense_date">
        ...
        <xsl:variable name="P_no_period" select="fsul:strip-period(P)"/>

        <xsl:variable name="date_after"
            select="if (matches($P_no_period,'defended this')) then (substring-after($P_no_period,'on ')) else ('NO DEFENSE DATE')"/>
<!--
        ...
        <xsl:variable name="date_no_abbrev">
            <xsl:value-of
                select="if (contains($date_after,'Jan ')) then replace($date_after,'Jan ','January ') else
