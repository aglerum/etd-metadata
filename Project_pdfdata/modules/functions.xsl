...
<xsl:stylesheet version="2.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:fsul="http://www.lib.fsu.edu" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl"
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p>This version ignores punctuation. Also note that only titles, names, or keywords in ALL CAPS or keywords in lower-case need to be
                modified.</xd:p>
    ...
    <xsl:function name="fsul:TitleCase">
        ...
        <xsl:variable name="letters"
            select="'[a-zA-ZÀÁĀĂĄǍǞǠǢǺǼȀȂĆĈĊČÐĎÉÊĒĔĖĘĚȄĜĞĠĢǤǦǴĤĦÍĨĪĬĮİĲǏĴĶǨĹĻĽĿŁÑŃŅŇŊǸÒÓÔÕÖØŌŎŐŒǑǪǬǾŔŖŘŚŜŞŠŢŤŦÙÚÛÜŨŪŬŮŰŲǓǕǗǙǛŴÝŶŸŹŻŽàáãäåæāăąǎǟǡǣǻǽȁȃçćĉċčďđèéêëēĕėęěȅĝğġģǥǧǵĥħìíîïĩīĭįıĳǐĵķĸǩĺļľŀłńņňŋǹðñòóôõöøōŏőœǒǫǭǿŕŗřśŝşšţťŧùúûüũūŭůűųǔǖǘǚǜŵýÿŷźżž]+'"
            as="xs:string"/>
            ...
            <xsl:matching-substring>
                <xsl:value-of
                    select="
                        ...
                        if (matches(., '^[a-zàáâãäåæāăąǎǟǡǣǻǽȁȃçćĉċčďđèéêëēĕėęěȅĝğġģǥǧǵĥħìíîïĩīĭįıĳǐĵķĸǩĺļľŀłñńņňŋǹðñòóôõöøōŏőœǒǫǭǿŕŗřśŝşšţťŧùúûüũūŭůűųǔǖǘǚǜŵýÿŷźżž]+'))
                            then
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p>Function to strip HTLML tags from the title and summary fields. This is necessary for transformation to MODS and MARC21 but not
                Bepress.</xd:p>
            <xd:p>Acknowledgement: Adapted from Joachim Selke: http://blog.joachim-selke.de/2011/01/stripping-html-tags-in-xslt/. Conditional updated
                to XPath 2.0.</xd:p>
        </xd:desc>
    </xd:doc>
    <!--    @param $string is the string to process  -->
    ...
    <!--** Work in Progress. Sometimes fsul:strip-tags does not strip all tags. ** -->    
    <xd:doc>
        <xd:desc>
            <xd:p>
                <xd:b>fsul:strip-specific-tags</xd:b>
            </xd:p>
            <xd:p>Function to strip specific HTLML tags from the title and summary fields. This is necessary for transformation to MODS and MARC21 but
                not Bepress.</xd:p>
        </xd:desc>
    </xd:doc>
    <!--    @param $string is the string to process  -->
    ...
    <xsl:function name="fsul:entity2html">
        ...
        <xsl:variable name="character-map" select="document('../tables/characters.xml')/character-map"/> <xsl:analyze-string select="$string"
            regex="&amp;#[0-9]{{3,4}};">
            <xsl:matching-substring>
                <xsl:value-of
                    select="
    ...
    <xd:doc>
        <xd:desc>
            ...
            <xd:p>
                <xd:b>Function to convert utf8 curly quotes characters found in Bepress metadata to straight quotes.</xd:b>
            </xd:p>
            <xd:p/>
        </xd:desc>
    </xd:doc>
    ...
    <xsl:function name="fsul:convert-curly">
        ...
        <xsl:variable name="apos">
            <xsl:text>&apos;</xsl:text>
        </xsl:variable> <xsl:variable name="single-entity">
        ...
        </xsl:variable> <xsl:variable name="single-character">
        ...
        </xsl:variable> <xsl:variable name="double-entity">
        ...
        </xsl:variable> <xsl:variable name="double-characters">
        ...
        </xsl:variable> <xsl:analyze-string select="$double-characters" regex="(\s¡°|¡±\s)">
            <xsl:matching-substring>
                <xsl:value-of select="replace(., '.', $quote)"/>
            </xsl:matching-substring>
            <xsl:non-matching-substring>
    ...
    <xsl:function name="fsul:characters2utf8">
        ...
        </xsl:variable> <xsl:value-of select="$latin-double"/> </xsl:function>
</xsl:stylesheet>
